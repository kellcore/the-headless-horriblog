...
const UnBlock = (props) => {
    ...
    const previousPage = currentPage - 1 === 1 ? '/category/un-block' : `/category/un-block${currentPage - 1}`;
    // does the current page - 1 equal 1? if so, direct to blog, otherwise, direct to blog/2 etc.
    const nextPage = `/category/un-block/${currentPage + 1}`;

    return (
        <Layout>
            <SEO title='Un Block' keywords={['halloween', 'horror blog', 'horror movies']} />
            <Nav />
...
export const pageQuery = graphql`
    query UnBlockQuery ($skip: Int!, $limit: Int!) {
        allContentfulBlog(
