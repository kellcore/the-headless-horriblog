...
const JohnCarpenterBlock = (props) => {
    ...
    const previousPage = currentPage - 1 === 1 ? '/category/john-carpenter-block' : `/category/john-carpenter-block${currentPage - 1}`;
    // does the current page - 1 equal 1? if so, direct to blog, otherwise, direct to blog/2 etc.
    const nextPage = `/category/john-carpenter-block/${currentPage + 1}`;

    return (
        <Layout>
            <SEO title='John Carpenter Block' keywords={['halloween', 'horror blog', 'horror movies']} />
            <Nav />
...
export default JohnCarpenterBlock;

export const pageQuery = graphql`
    query JohnCarpenterBlockQuery ($skip: Int!, $limit: Int!) {
        allContentfulBlog(
