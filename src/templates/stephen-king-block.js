...
const StephenKingBlock = (props) => {
    ...
    const previousPage = currentPage - 1 === 1 ? '/category/stephen-king-block' : `/category/stephen-king-block${currentPage - 1}`;
    // does the current page - 1 equal 1? if so, direct to blog, otherwise, direct to blog/2 etc.
    const nextPage = `/category/stephen-king-block/${currentPage + 1}`;

    return (
        <Layout>
            <SEO title='Stephen King Block' keywords={['halloween', 'horror blog', 'horror movies']} />
            <Nav />
...
export default StephenKingBlock;

export const pageQuery = graphql`
    query StephenKingBlockQuery ($skip: Int!, $limit: Int!) {
        allContentfulBlog(
