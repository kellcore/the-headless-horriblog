...
const VampireBlock = (props) => {
    ...
    const previousPage = currentPage - 1 === 1 ? '/category/vampire-block' : `/category/vampire-block${currentPage - 1}`;
    // does the current page - 1 equal 1? if so, direct to blog, otherwise, direct to blog/2 etc.
    const nextPage = `/category/vampire-block/${currentPage + 1}`;

    return (
        <Layout>
            <SEO title='Vampire Block' keywords={['halloween', 'horror blog', 'horror movies']} />
            <Nav />
...
export default VampireBlock;

export const pageQuery = graphql`
    query VampireBlockQuery ($skip: Int!, $limit: Int!) {
        allContentfulBlog(
