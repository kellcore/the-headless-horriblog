...
const AwfulBlock = (props) => {
    ...
    const previousPage = currentPage - 1 === 1 ? '/category/awful-block' : `/category/awful-block${currentPage - 1}`;
    // does the current page - 1 equal 1? if so, direct to blog, otherwise, direct to blog/2 etc.
    const nextPage = `/category/awful-block/${currentPage + 1}`;

    return (
        <Layout>
            <SEO title='Awful Block' keywords={['halloween', 'horror blog', 'horror movies']} />
            <Nav />
...
export default AwfulBlock;

export const pageQuery = graphql`
    query AwfulBlockQuery ($skip: Int!, $limit: Int!) {
        allContentfulBlog(
