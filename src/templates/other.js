...
const Other = (props) => {
    ...
    const previousPage = currentPage - 1 === 1 ? '/category/other' : `/category/other/${currentPage - 1}`;
    // does the current page - 1 equal 1? if so, direct to blog, otherwise, direct to blog/2 etc.
    const nextPage = `/category/other/${currentPage + 1}`;

    return (
        <Layout>
            <SEO title='Other' keywords={['halloween', 'horror blog', 'horror movies']} />
            <Nav />
...
export const pageQuery = graphql`
    query OtherQuery ($skip: Int!, $limit: Int!) {
        allContentfulBlog(
