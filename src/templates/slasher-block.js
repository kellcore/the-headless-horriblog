...
const SlasherBlock = (props) => {
    ...
    const previousPage = currentPage - 1 === 1 ? '/category/slasher-block' : `/category/slasher-block${currentPage - 1}`;
    // does the current page - 1 equal 1? if so, direct to blog, otherwise, direct to blog/2 etc.
    const nextPage = `/category/slasher-block/${currentPage + 1}`;

    return (
        <Layout>
            <SEO title='Slasher Block' keywords={['halloween', 'horror blog', 'horror movies']} />
            <Nav />
...
export default SlasherBlock;

export const pageQuery = graphql`
    query SlasherBlockQuery ($skip: Int!, $limit: Int!) {
        allContentfulBlog(
