...
const JapaneseBlock = (props) => {
    ...
    const previousPage = currentPage - 1 === 1 ? '/category/japanese-block' : `/category/japanese-block${currentPage - 1}`;
    // does the current page - 1 equal 1? if so, direct to blog, otherwise, direct to blog/2 etc.
    const nextPage = `/category/japanese-block/${currentPage + 1}`;

    return (
        <Layout>
            <SEO title='Japanese Block' keywords={['halloween', 'horror blog', 'horror movies']} />
            <Nav />
...
export default JapaneseBlock;

export const pageQuery = graphql`
    query JapaneseBlockQuery ($skip: Int!, $limit: Int!) {
        allContentfulBlog(
